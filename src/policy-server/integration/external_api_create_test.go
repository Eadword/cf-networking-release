...
var _ = Describe("External API Adding Policies", func() {
	...
	FDescribe("adding egress policies", func() {
		addPoliciesSucceeds := func(version, request string) {
			body := strings.NewReader(request)
			resp := helpers.MakeAndDoRequest(
				"POST",
				fmt.Sprintf("http://%s:%d/networking/%s/external/policies", conf.ListenHost, conf.ListenPort, version),
		...
		v1Request := `{ "policies": [ {"source": { "type": "app", "id": "some-app-guid" }, "destination": { "type": "ip" , "ips": [{"start": "10.27.1.1", "end": "10.27.1.2"}], "protocol": "tcp", "ports": { "start": 8090, "end": 8090 } } } ] }`

		DescribeTable("adding policies succeeds", addPoliciesSucceeds,
			Entry("v1", "v1", v1Request),
		)
	})
})
